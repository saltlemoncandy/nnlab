...
def train(model, encoder:du.Encoder.AbstractEncoder, trainDatasetX, trainDatasetY, testDatasetX, testDatasetY):
    ...
    index_loss_fn = WeightedMSELoss(du.Encoder.SparseEntryEncoder.NULL_INDEX, weightNormalIndex)
...
def __testing():
    ...
    readSize = 3 # use how many data in training and testing. integer, -1~10000
    ...
    encoder = du.Encoder.SparseEntryEncoder(inputSetSize, outputSetSize, nonFloatIndices = nonFloatIndices, maxInputNum = maxInputNum)
    ...
    #trainDatasetX[0] = (trainDatasetX[0] / maxXForNormalization).astype('float16', copy=False)
    ...
    with open(testDatasetFilePath,"r") as testDatasetFile:
        ...
        #testDatasetX[0] = (testDatasetX[0] / maxXForNormalization).astype('float16', copy=False)
    ...
    #print(trainDatasetX[0])
    #print(trainDatasetX[1])
    #print(trainDatasetX[1].shape)
    # trainDatasetX shape = [readSize, encoder.getInputDim()] + [readSize, max_length] * len(nonFloatIndices)) 
    ...
    embedding_dim = 8
    model = SparseModel(encoder, hidden_dim, embedding_dim)
    model.to(device=DEVICE)
    #
    #test_input1 = torch.tensor([[-15.0, 95.0, 129.0, -116.0], [1.0, 2.0, 3.0, 4.0]], device=DEVICE)
    #test_input2 = torch.tensor([[2,3],[4,5]], device=DEVICE)
